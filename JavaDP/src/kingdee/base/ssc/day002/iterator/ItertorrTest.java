 ...
 * Iterator翻译成汉语就是迭代器的意思。提到迭代器，首先它是与集合相关的，集合也叫聚集、容器等，我们可以将集合看成是一个可以包容对象的容器，
 ...
 * 上面的代码中，Aggregate是容器类接口，大家可以想象一下Collection，List，Set等，Aggregate就是他们的简化版，容器类接口中主要有三个方法：
 * 添加对象方法add、删除对象方法remove、取得迭代器方法iterator。Iterator是迭代器接口，主要有两个方法：取得迭代对象方法next，
 * 判断是否迭代完成方法hasNext，大家可以对比java.util.List和java.util.Iterator两个接口自行思考。
 * 迭代器模式的优点：
 *
 ...
 *         可以提供多种遍历方式，比如说对有序列表，我们可以根据需要提供正序遍历，倒序遍历两种迭代器，用户用起来只需要得到我们实现好的迭代器，
 *         就可以方便的对集合进行遍历了。
...
public class ItertorrTest {
    @Test
    public void test(){
        Aggregate aggregate = new ConcreateAggregate();
        aggregate.add(new JavaBean("leipq"));
        aggregate.add(new JavaBean("chengsipeng"));
        Itertorr itertorr = aggregate.itertorr();
        while(itertorr.hasNext())
            System.out.println(itertorr.next().toString());
